...
const createAuditTable = () => {
  const sentence = `create table if not exists ${AUDITORIA.NAME} (${AUDITORIA.KEY_1} text primary key not null,${AUDITORIA.ID_PRECIADOR} text ,${AUDITORIA.ID_PERCHA} text ,${AUDITORIA.ID_PROMOCION} text ,${AUDITORIA.ID_SUCURSAL} text ,${AUDITORIA.ID_CLIENTE} TEXT ,${AUDITORIA.ID_PORTAFOLIO_AUDITORIA} text ,${AUDITORIA.NOMBRE_CLIENTE} text ,${AUDITORIA.NOMBRE_SUCURSAL} text ,${AUDITORIA.USUARIO_CREACION} VARCHAR(43) ,${AUDITORIA.FECHA_CREACION} VARCHAR(43) ,${AUDITORIA.SINCRONIZADA} SMALLINT ,${AUDITORIA.ID_GRUPO_CLIENTE} TEXT ) `;
  // const sentenceIndex = `CREATE INDEX ${CLIENTE_TABLE.INDEX_1} ON ${CLIENTE_TABLE.TABLE_NAME} (${CLIENTE_TABLE.ITEM_6});`
